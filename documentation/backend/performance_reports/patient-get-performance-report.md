...
| Métrica                | Valor                   |
| ---------------------- | ----------------------- |
| **Promedio (avg)**     | 1.312 ms                |
| **Mínimo (min)**       | 0.600 ms                |
| **Mediana (med)**      | 1.287 ms                |
| **Máximo (max)**       | 9.512 ms                |
| **Percentil 90 (p90)** | 1.825 ms                |
| **Percentil 95 (p95)** | 2.075 ms ✅ `< 2000 ms` |
| **Percentil 99 (p99)** | 3.304 ms                |

> Los tiempos de respuesta son notablemente bajos. La mediana de 1.287 ms indica que el 50% de las peticiones fueron atendidas en poco más de 1 ms, lo que refleja un endpoint altamente eficiente respaldado por una consulta de base de datos bien optimizada. El máximo registrado de 9.512 ms no compromete en absoluto el rendimiento general.

#### Rendimiento del Sistema (TPS/RPS)

| Métrica                              | Valor                      |
| ------------------------------------ | -------------------------- |
| **Total de peticiones HTTP (test)**  | 2,728                      |
| **Total de peticiones HTTP (setup)** | 10                         |
| **Total de peticiones HTTP**         | 2,738                      |
| **Tasa de peticiones (rate)**        | 34.152 req/s ✅ `≥ 25 TPS` |
| **Total de iteraciones**             | 2,728                      |
| **Tasa de iteraciones**              | 34.027 iter/s              |
| **Duración media de iteración**      | 1,002.0 ms                 |
| **VUs máximos (vus_max)**            | 45                         |

> La tasa de 34.152 req/s supera el mínimo requerido de 25 TPS. Las 10 peticiones adicionales de la fase de `setup` corresponden a la preparación de datos de prueba (creación de pacientes de referencia) y no se contabilizan en los umbrales funcionales.
...
| Recurso                           | Observación                                                                                                                                                                    |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **CPU**                           | No instrumentada directamente. La latencia media de 1.312 ms por petición GET indica que el procesamiento en servidor y el acceso a BD son mínimos, con carga de CPU muy baja. |
...
| **Red — Datos recibidos**         | 727,174 bytes (≈ 710 KB total) a **8.99 KB/s** promedio. Aproximadamente **267 bytes por respuesta**.                                                                           |
| **Red — Datos recibidos (setup)** | ~10,650 bytes adicionales de la fase de setup.                                                                                                                                   |
| **Red — Datos enviados**          | 264,943 bytes (≈ 259 KB total) a **3.23 KB/s** promedio.                                                                                                                        |
...
| **Máxima tasa de peticiones sostenida** | 34.152 req/s                                                                                        |
...
> El endpoint de consulta individual presenta el mejor perfil de escalabilidad del sistema. Al ser una operación de solo lectura con payload de respuesta pequeño (~267 bytes), su capacidad de manejo de carga es alta. El límite de capacidad real no fue determinado en esta prueba de carga normal.
...
> No se registró ningún error en las 2,738 peticiones totales ejecutadas (2,728 del test principal y 10 del setup). La tasa de error es exactamente 0%.

### Códigos de Estado HTTP

| Código             | Cantidad | Descripción                                   |
| ------------------ | -------- | --------------------------------------------- |
| **2xx (éxito)**    | 2,738    | Todas las peticiones finalizaron exitosamente |
...
1. **El endpoint GET de consulta de paciente por ID es el de mayor rendimiento del sistema**, con una mediana de respuesta de 1.287 ms y un p95 de 2.075 ms, superando el umbral de 2000 ms por más de 960 veces.
2. **La tasa de 34.152 req/s** supera el mínimo requerido de 25 TPS, confirmando que el sistema puede soportar la demanda esperada del flujo de consulta durante las operaciones de triage.
3. **El payload de respuesta promedio de 267 bytes** es muy compacto y eficiente, minimizando el consumo de ancho de banda incluso ante cargas elevadas.
4. **No se registró ningún error** en las 2,738 peticiones (0% de tasa de error), confirmando la fiabilidad absoluta del endpoint bajo carga normal.
...
6. **Comparar el p95 de producción vs. p95 de prueba** (2.075 ms) periódicamente. Una desviación sostenida mayor a 10x podría indicar problemas de infraestructura, saturación de conexiones a la BD o ausencia de índices.
