...
const serviceHoursLevel = {
  [criticityLevel.EMERGENCIA]: {
    color: "🔴 Rojo",
    denomination: "Emergencia",
    maxAttentionTime: "0 minutos",
  },
  [criticityLevel.MUY_URGENTE]: {
    color: "🟠 Naranja",
    denomination: "Muy Urgente",
    maxAttentionTime: "10 minutos",
  },
  [criticityLevel.URGENTE]: {
    color: "🟡 Amarillo",
    denomination: "Urgente",
    maxAttentionTime: "60 minutos",
  },
  [criticityLevel.MENOS_URGENTE]: {
    color: "🟢 Verde",
    denomination: "Menos Urgente",
    maxAttentionTime: "120 minutos",
  },
  [criticityLevel.NO_URGENTE]: {
    color: "🔵 Azul",
    denomination: "No Urgente",
    maxAttentionTime: "240 minutos",
  },
};

function classifyHeartRate(hr) {
  if (hr < 40 || hr >= 150) return criticityLevel.EMERGENCIA;
  if ((hr >= 40 && hr <= 49) || (hr >= 130 && hr <= 149))
    return criticityLevel.MUY_URGENTE;
  if ((hr >= 50 && hr <= 59) || (hr >= 101 && hr <= 129))
    return criticityLevel.URGENTE;
  return criticityLevel.NO_URGENTE;
}

function classifyRespiratoryRate(rr) {
  if (rr < 8 || rr > 30) return criticityLevel.EMERGENCIA;
  if ((rr >= 8 && rr <= 10) || (rr >= 26 && rr <= 30))
    return criticityLevel.MUY_URGENTE;
  if ((rr >= 11 && rr <= 12) || (rr >= 21 && rr <= 25))
    return criticityLevel.URGENTE;
  return criticityLevel.NO_URGENTE;
}

function classifyOxygenSaturation(spo2) {
  if (spo2 < 85) return criticityLevel.EMERGENCIA;
...
function classifyTemperature(temp) {
  if (temp < 32.0 || temp > 40.0) return criticityLevel.EMERGENCIA;
  if ((temp >= 32.0 && temp <= 34.9) || (temp >= 39.1 && temp <= 40.0))
    return criticityLevel.MUY_URGENTE;
  if ((temp >= 35.0 && temp <= 35.9) || (temp >= 38.5 && temp <= 39.0))
    return criticityLevel.URGENTE;
...
function classifyBloodPressure(systolic, diastolic) {
  ...
  if (systolic < 70 || systolic > 200) sl = criticityLevel.EMERGENCIA;
  else if (
    (systolic >= 70 && systolic <= 89) ||
    (systolic >= 181 && systolic <= 200)
  )
    sl = criticityLevel.MUY_URGENTE;
  else if (
    (systolic >= 90 && systolic <= 99) ||
    (systolic >= 161 && systolic <= 180)
  )
    sl = criticityLevel.URGENTE;
  else if (systolic >= 140 && systolic <= 160)
    sl = criticityLevel.MENOS_URGENTE;
  else if (systolic >= 100 && systolic <= 139) sl = criticityLevel.NO_URGENTE;
  else sl = criticityLevel.NO_URGENTE;

  if (diastolic > 130) dl = criticityLevel.EMERGENCIA;
  else if (diastolic >= 111 && diastolic <= 130)
    dl = criticityLevel.MUY_URGENTE;
  else if (diastolic >= 101 && diastolic <= 110) dl = criticityLevel.URGENTE;
  else if (diastolic >= 90 && diastolic <= 100)
    dl = criticityLevel.MENOS_URGENTE;
...
function classifyConsciousnessLevel(consciousness) {
  switch (consciousness) {
    case consciousnessLevel.SIN_RESPUESTA:
      return criticityLevel.EMERGENCIA;
    case consciousnessLevel.RESPONDE_DOLOR:
      return criticityLevel.MUY_URGENTE;
    case consciousnessLevel.RESPONDE_VOZ:
      return criticityLevel.URGENTE;
    case consciousnessLevel.CONFUSO:
      return criticityLevel.MENOS_URGENTE;
    case consciousnessLevel.ALERTA:
      return criticityLevel.NO_URGENTE;
    default:
      throw new Error(`Nivel de conciencia inválido: "${consciousness}"`);
  }
}
...
function validateVitalSigns(signs) {
  ...
  const rules = [
    {
      valid:
        signs.frecuencia_cardiaca >= 20 && signs.frecuencia_cardiaca <= 300,
      msg: `Frecuencia cardíaca fuera de rango (20-300): ${signs.frecuencia_cardiaca}`,
    },
    {
      valid:
        signs.frecuencia_respiratoria >= 1 &&
        signs.frecuencia_respiratoria <= 60,
    ...
    {
      valid: !(
        signs.nivel_de_conciencia === consciousnessLevel.SIN_RESPUESTA &&
        signs.nivel_de_dolor !== 0
      ),
...
export function classifyTriage(signs) {
  ...
  const respiratoryRateLevel = classifyRespiratoryRate(
    signs.frecuencia_respiratoria,
  );
  ...
  const consciousnessLevelVal = classifyConsciousnessLevel(
    signs.nivel_de_conciencia,
  );
  ...
  if (respiratoryRateLevel === level)
    criticalSigns.push("Frecuencia Respiratoria");
  ...
  if (consciousnessLevelVal === level)
    criticalSigns.push("Nivel de Conciencia");
