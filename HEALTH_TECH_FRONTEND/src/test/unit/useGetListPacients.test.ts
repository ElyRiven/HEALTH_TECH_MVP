...
describe("useGetListPacients", () => {
  ...
  it("maps criticidad 1 → Emergencia / manchester-criticidad-1", async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: () => Promise.resolve([{ ...basePacient, criticidad: 1 }]),
    });
    ...
    expect(result.current.pacients[0].criticidadColor).toBe(
      "manchester-criticidad-1",
    );
  });
