...
describe('PatientTable', () => {
  ...
  it('shows register link in empty state', () => {
    setHookState({ pacients: [] })
    render(<PatientTable />)
    const link = screen.getByRole('link', { name: /registra tu primer paciente/i })
      ...
      pacients: [
        {
          ...
          apellidos: 'González',
          criticidad: 1,
          criticidadDenominacion: 'Inmediato',
  ...
  it('table header shows Nombres, Apellidos, Criticidad columns', () => {
    ...
    setHookState({
      pacients: [{
        identificacion: 1, nombres: 'X', apellidos: 'Y', criticidad: 1,
        criticidadDenominacion: 'Inmediato', criticidadColor: '',
        fecha_de_nacimiento: '2000-01-01', genero: 'hombre',
        hora_de_registro: '10:00', estado: 'En espera',
      }],
    })
    render(<PatientTable />)
  ...
  it('sort button calls fetchPacients with toggled order', async () => {
    setHookState({
      pacients: [{
        identificacion: 1, nombres: 'Juan', apellidos: 'Perez', criticidad: 1,
        criticidadDenominacion: 'Inmediato', criticidadColor: '',
        fecha_de_nacimiento: '1990-01-01', genero: 'hombre',
        hora_de_registro: '10:00', estado: 'En espera',
      }],
    })
    render(<PatientTable />)
  ...
  it('sort button toggles back to ASC on second click', async () => {
    ...
    fireEvent.click(sortBtn)
    fireEvent.click(sortBtn)
    await waitFor(() => expect(mockFetchPacients).toHaveBeenLastCalledWith('ASC'))
  })
})
