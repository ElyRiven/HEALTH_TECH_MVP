...
function fillForm(
  ...
  Object.entries(data).forEach(([name, value]) => {
    act(() => {
      result.current.handleChange({ target: { name, value } } as React.ChangeEvent<HTMLInputElement>)
    })
  })
}
...
describe('useFormRegisterVitals', () => {
  ...
  it('returns correct initial state', () => {
    const { result } = renderHook(() => useFormRegisterVitals('123'), { wrapper })
  ...
  it('errors when frecuencia_cardiaca is empty', async () => {
    ...
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toEqual(expect.arrayContaining(['Frecuencia cardíaca: campo requerido']))
  })
  ...
  it('errors when frecuencia_cardiaca has decimals', async () => {
    ...
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toEqual(expect.arrayContaining(['Frecuencia cardíaca: debe ser un número entero (sin decimales)']))
  })
  ...
  it('errors when frecuencia_cardiaca is below range', async () => {
    ...
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toEqual(expect.arrayContaining(['Frecuencia cardíaca: debe estar entre 20 y 300 bpm']))
  })
  ...
  it('errors when frecuencia_respiratoria is empty', async () => {
    ...
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toEqual(expect.arrayContaining(['Frecuencia respiratoria: campo requerido']))
  })
  ...
  it('errors when frecuencia_respiratoria is not integer', async () => {
    ...
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toEqual(expect.arrayContaining(['Frecuencia respiratoria: debe ser un número entero (sin decimales)']))
  })
  ...
  it('errors when frecuencia_respiratoria is out of range', async () => {
    ...
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toEqual(expect.arrayContaining(['Frecuencia respiratoria: debe estar entre 1 y 60 rpm']))
  })
  ...
  it('errors when saturacion_o2 is empty', async () => {
    ...
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toEqual(expect.arrayContaining(['Saturación O2: campo requerido']))
  })
  ...
  it('errors when saturacion_o2 has more than 1 decimal', async () => {
    ...
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toEqual(expect.arrayContaining(['Saturación O2: admite como máximo un decimal (ej: 98.5)']))
  })
  ...
  it('errors when saturacion_o2 is out of range', async () => {
    ...
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toEqual(expect.arrayContaining(['Saturación O2: debe estar entre 50.0 y 100.0 %']))
  })
  ...
  it('errors when temperatura is empty', async () => {
    ...
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toEqual(expect.arrayContaining(['Temperatura: campo requerido']))
  })
  ...
  it('errors when temperatura has more than 1 decimal', async () => {
    ...
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toEqual(expect.arrayContaining(['Temperatura: admite como máximo un decimal (ej: 36.5)']))
  })
  ...
  it('errors when temperatura is out of range', async () => {
    ...
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toEqual(expect.arrayContaining(['Temperatura: debe estar entre 25.0 y 45.0 °C']))
  })
  ...
  it('errors when presion is empty', async () => {
    ...
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toEqual(expect.arrayContaining(['Presión arterial: campo requerido']))
  })
  ...
  it('errors when presion has invalid format', async () => {
    ...
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toEqual(expect.arrayContaining([expect.stringContaining('formato inválido')]))
  })
  ...
  it('errors when sistolica is out of range', async () => {
    ...
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toEqual(expect.arrayContaining(['Presión arterial: la sistólica debe estar entre 50 y 300 mmHg']))
  })
  ...
  it('errors when diastolica is out of range', async () => {
    ...
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toEqual(expect.arrayContaining(['Presión arterial: la diastólica debe estar entre 20 y 200 mmHg']))
  })
  ...
  it('errors when diastolica >= sistolica', async () => {
    ...
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toEqual(expect.arrayContaining(['Presión arterial: la diastólica debe ser menor que la sistólica']))
  })
  ...
  it('errors when nivel_de_dolor is empty', async () => {
    ...
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toEqual(expect.arrayContaining(['Nivel de dolor: campo requerido']))
  })
  ...
  it('errors when nivel_de_dolor is not integer', async () => {
    ...
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toEqual(expect.arrayContaining(['Nivel de dolor: debe ser un número entero']))
  })
  ...
  it('errors when nivel_de_dolor is out of range', async () => {
    ...
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toEqual(expect.arrayContaining(['Nivel de dolor: debe estar entre 0 y 10 (escala EVA)']))
  })

  it('errors when nivel_de_conciencia is Sin respuesta and dolor != 0', async () => {
    const { result } = renderHook(() => useFormRegisterVitals('123'), { wrapper })
    fillForm(result, { nivel_de_conciencia: 'Sin respuesta', nivel_de_dolor: '5' })
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toEqual(
      expect.arrayContaining(["Nivel de dolor: debe ser 0 cuando el nivel de conciencia es 'Sin respuesta'"])
    )
  })
  ...
  it('accepts nivel_de_dolor = 0 when nivel_de_conciencia is Sin respuesta', async () => {
    ...
    const { result } = renderHook(() => useFormRegisterVitals('123'), { wrapper })
    fillForm(result, { nivel_de_conciencia: 'Sin respuesta', nivel_de_dolor: '0' })
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    expect(result.current.formError).toBeNull()
  })
  ...
  it('submits successfully and navigates to /', async () => {
    ...
    const { result } = renderHook(() => useFormRegisterVitals('123', onSuccess), { wrapper })
    fillForm(result)
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
    ...
    await waitFor(() => expect(onSuccess).toHaveBeenCalledWith('Signos vitales registrados'))
  })

  it('handles API error with errores object', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: false,
      json: () => Promise.resolve({ errores: { campo: 'Error en campo vital' } }),
    })
    const { result } = renderHook(() => useFormRegisterVitals('123'), { wrapper })
    fillForm(result)
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
  ...
  it('handles API error with campos_faltantes', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: false,
      json: () => Promise.resolve({ campos_faltantes: ['frecuencia_cardiaca'] }),
    })
    const { result } = renderHook(() => useFormRegisterVitals('123'), { wrapper })
    fillForm(result)
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
  ...
  it('calls onError through effect when formError is set', async () => {
    const onError = vi.fn()
    const { result } = renderHook(() => useFormRegisterVitals('123', undefined, onError), { wrapper })
    fillForm(result, { frecuencia_cardiaca: '' })
    await act(async () => { await result.current.handleSubmit({ preventDefault: vi.fn() } as unknown as React.FormEvent<HTMLFormElement>) })
