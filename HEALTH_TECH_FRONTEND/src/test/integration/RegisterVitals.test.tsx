...
describe('RegisterVitals page (integration)', () => {
  ...
  it('shows initial success alert for newly registered patient', () => {
    ...
    expect(screen.getByText(/paciente con identificación 456 registrado exitosamente/i)).toBeInTheDocument()
  })
  ...
  it('shows multiple validation errors', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /guardar/i }))
    await waitFor(() => {
      const errors = screen.getAllByText(/frecuencia cardíaca: campo requerido/i)
  ...
  it('submits successfully and navigates to /', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/frecuencia cardiaca/i), { target: { name: 'frecuencia_cardiaca', value: '75' } })
    fireEvent.change(screen.getByLabelText(/frecuencia respiratoria/i), { target: { name: 'frecuencia_respiratoria', value: '18' } })
    fireEvent.change(screen.getByLabelText(/saturación o2/i), { target: { name: 'saturacion_o2', value: '98' } })
    fireEvent.change(screen.getByLabelText(/temperatura/i), { target: { name: 'temperatura', value: '37' } })
    fireEvent.change(screen.getByLabelText(/presión/i), { target: { name: 'presion', value: '120/80' } })
    fireEvent.change(screen.getByLabelText(/nivel de dolor/i), { target: { name: 'nivel_de_dolor', value: '3' } })
