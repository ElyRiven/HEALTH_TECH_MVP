  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!fr) {
      fieldErrors.frecuencia_respiratoria =
        "Frecuencia respiratoria: campo requerido";
    } else if (!/^\d+$/.test(fr)) {
      fieldErrors.frecuencia_respiratoria =
    ...
    if (!temp) {
      ...
      if (val < 25 || val > 45)
        fieldErrors.temperatura =
          "Temperatura: debe estar entre 25.0 y 45.0 °C";
    }
    ...
    if (!presion) {
      ...
      fieldErrors.presion =
        "Presión arterial: formato inválido. Use 'sistólica/diastólica' con 2-3 dígitos (ej: 120/80)";
    } else {
    ...
    try {
      ...
      const successMessage =
        data.message || "Signos vitales registrados exitosamente";
